...
def InitialiseOptions(Config):
    ...
    for key in input_pars.keys():
        ...
        # Input
        if (key == 'output') or (key == 'run-type') or (key == 'selection-effects-cut') or (key == 'SNR-method') or (key == 'observing-run') or (key == 'PSD-path') or (key == 'sampling-method'):
...
\nWelcome to the ICAROGW simulation pipeline helper!\n
    ...
    # ----- #
        ...
        sampling-method               [str  ]  Sampling method to use to draw events. Options: rejection (for rejection sampling), cumulative (samples are drawn uniformly in [0, 1] then converted to parameter space via inverse CDF). Default: rejection.
