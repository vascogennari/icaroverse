...
class BilbyDetectionPipeline():
    ...
    def set_ifos_and_inject_signal(self):
        ...
        if self.precessing_apx:
            ...
            projected_event_dict['a_1']    = self.event_dict['a_1'] * abs(self.event_dict['costilt_1'])
            projected_event_dict['a_2']    = self.event_dict['a_2'] * abs(self.event_dict['costilt_2'])
            projected_event_dict['tilt_1'] = np.arccos(np.sign(self.event_dict['costilt_1']))
            projected_event_dict['tilt_2'] = np.arccos(np.sign(self.event_dict['costilt_2']))
            self.ifos_list.inject_signal(
                waveform_generator = self.waveform_generator,
